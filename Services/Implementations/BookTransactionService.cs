...
{
    public class BookTransactionService(LibraryContext context, ILogger<BookTransactionService> logger, IBookStatusService bookStatusService, IConfiguration configuration) : BaseService<BookTransactionService>(context, logger), IBookTransactionService
    {
        ...
        public async Task<GenericResponse<ReservationNotification?>> SaveReservationNotificationAsync(ReservationNotification notification)
        {
            try
            {
                _context.ReservationNotifications.Add(notification);
                bool success = await _context.SaveChangesAsync() > 0;

                return new GenericResponse<ReservationNotification?>
                {
                    Response = success ? notification : null,
                    Success = success,
        ...
        public async Task<GenericResponse<bool>> DisableNotificationAsync(int notificationId)
        {
            try
            {
                ...
                _context.ReservationNotifications.Update(notification);
                bool success = await _context.SaveChangesAsync() > 0;
